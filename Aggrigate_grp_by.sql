...
select count(*) from sales_data where category='Electronics';

-- Total quantity sold
select sum(quantity) from sales_data;

-- Total revenue generated 
select sum(quantity * price) as revenue from sales_data;
...
-- Which category has the highest maximum price among all categories
 select category,max(price) from sales_data group by category limit 1;

-- Total orders per year
 select sale_date,count(sale_id) from sales_data group by sale_date having year(sale_date);

-- Find the number of sales in each region and display only those regions where the total sales greater than 12.
 select region,count(*) from sales_data group by region having count(*) > 12;
